# test_arch — architecture project

Instance data (subscriptions, networks, components, blocks, scenarios,
containers, applications) authored in `landscape.todl` against the
`tech-architecture` meta-model (`../../meta-models/tech-architecture/`),
using the `microsoft` and `aws` libraries (`../../libraries/`).

...
1. **`Cardinality` rename.** Live enum member is `NonEmpty` (`src/model/graph.ts:55`); all specs use `OneOrMore`. Rename `NonEmpty → OneOrMore` — **owner SPEC-01**. Until done, SPEC-05 refs won't compile.

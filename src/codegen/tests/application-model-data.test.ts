...
{
    const { root, shards } = ApplicationModelData.WithSoleModelFallback(Fixtures.SoleModel());
...
{
    assert.throws(() => ApplicationModelData.WithSoleModelFallback(Fixtures.TwoModels()), /application root/);
});

test("Strict throws when there is no entrypoint even for a sole model", () =>
{
    assert.throws(() => ApplicationModelData.Strict(Fixtures.SoleModel()), /entrypoint/);
});

...
export function generateReadClient(repo: Repository, options: ReadClientOptions): string
{
  ...
  const header =
    ...
    `import { ReflectedRepository, ReflectedEntity } from "${importSpecifier}";\n` +
    `import type { EntityReader, InstanceDescriptor, NodeId, Scalar, TodlDocument } from "${importSpecifier}";`;
...
): string
{
  ...
  return `export class ${className} extends ReflectedRepository {\n${fromJSON}\n\n${createEntity}\n\n${members}\n}`;
}

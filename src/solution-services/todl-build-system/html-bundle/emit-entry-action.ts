...
// compiled UI with the model DTO as its DataContext. Unlike GenerateModelDtoAction
// and GenerateAppUiAction, this template is static — it does not depend on the
// compiled model's shape, only on the manifest's id/name — and it is never hand-
// edited, so it belongs in ctx.Sandbox (build glue), not ctx.Project (project
// content).

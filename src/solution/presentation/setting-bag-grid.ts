import { GridProperty, PropertyKind, SettingKind } from '@pragmatic-tech-ai/mural/framework';
import {
    MapPropertyBag,
    type IPropertyBag,
        ...
        return bag.Definition.Fields.map((f) => {
            ...
            switch (SettingBagGrid.kindOf(f.Kind))
            {
                ...
                case PropertyKind.Number:
                    return GridProperty.number(f.Key, opts);
                case PropertyKind.Enum:
                    return GridProperty.enumOf(f.Key, f.Choices ? f.Choices.ToArray() : [], opts);
                case PropertyKind.Color:
                    return GridProperty.color(f.Key, opts);
                default:
